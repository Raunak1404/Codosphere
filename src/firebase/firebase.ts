...
// Update rank points for match winner and loser - IMPROVED VERSION
export const updateMatchResults = async (winnerId: string, loserId: string, matchId?: string) => {
  try {
    console.log("Updating match results - Winner:", winnerId, "Loser:", loserId, "Match:", matchId);
    
    // If matchId is provided, check if that specific match has already been processed
    if (matchId) {
      ...
      if (matchDoc.exists()) {
        ...
        if (matchData.pointsAwarded === true) {
          console.log(`Match ${matchId} already processed, not awarding points again`);
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (winnerDoc.exists()) {
        ...
        console.log(`Winner ${winnerId} current stats:`, { 
          currentRankPoints, 
          rankWins, 
          rankMatches 
        });
        ...
        console.log(`Winner ${winnerId} updated stats:`, { 
          totalRankPoints: currentRankPoints + 1, 
          rankWins: rankWins + 1, 
          rankMatches: rankMatches + 1 
        });
      } else {
        console.log(`Creating new document for winner ${winnerId}`);
      ...
      if (loserDoc.exists()) {
        ...
        console.log(`Loser ${loserId} current matches:`, rankMatches);
        ...
        console.log(`Loser ${loserId} updated matches:`, rankMatches + 1);
      } else {
        console.log(`Creating new document for loser ${loserId}`);
